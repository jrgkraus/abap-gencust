*---------------------------------------------------------------------*
*    program for:   VIEWFRAME_ZVI_GENCUSTIT
*---------------------------------------------------------------------*
FUNCTION VIEWFRAME_ZVI_GENCUSTIT       .
...
*----------------------------------------------------------------------*
CALL FUNCTION 'VIEWPROC_ZVI_GENCUSTIT'
         EXPORTING
              FCODE          = READ
...
EXTRACT = ZVI_GENCUSTIT_EXTRACT
TOTAL = ZVI_GENCUSTIT_TOTAL
              X_HEADER       = X_HEADER
              X_NAMTAB       = X_NAMTAB
...
CALL FUNCTION 'VIEWPROC_ZVI_GENCUSTIT'
             ...
             IMPORTING
                  UCOMM           = FUNCTION
UPDATE_REQUIRED = STATUS_ZVI_GENCUSTIT-UPD_FLAG
             TABLES
                  EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = ZVI_GENCUSTIT_EXTRACT
TOTAL = ZVI_GENCUSTIT_TOTAL
...
*----------------------------------------------------------------------*
      ...
      IF ( FUNCTION EQ SWITCH_TO_SHOW_MODE OR
           ...
           FUNCTION EQ END ) AND
STATUS_ZVI_GENCUSTIT-UPD_FLAG NE SPACE.
        PERFORM BEENDEN.
        CASE SY-SUBRC.
          WHEN 0.
CALL FUNCTION 'VIEWPROC_ZVI_GENCUSTIT'
...
TOTAL = ZVI_GENCUSTIT_TOTAL
            ...
            CASE SY-SUBRC.
              WHEN 0.
IF STATUS_ZVI_GENCUSTIT-UPD_FLAG EQ SPACE. EXIT. ENDIF.
...
IF STATUS_ZVI_GENCUSTIT-UPD_FLAG NE SPACE.
